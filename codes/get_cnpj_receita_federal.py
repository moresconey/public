    ...
    def upload_to_postgresql(self):
        ...
        for file in self.files:
            ...
            for chunk in pd.read_csv(temp_file, delimiter=';', header=None, chunksize=65000, names=list(layout_files[model]['columns'].keys()), iterator=True, dtype=str, encoding="ISO-8859-1"):
                ...
                try:
                    chunk.to_sql(layout_files[model]['table_name_db'], self.engine, if_exists="append", index=False, method= DB_CNPJ.psql_insert_copy)
                except:
                    time.sleep(60)
                    chunk.to_sql(layout_files[model]['table_name_db'], self.engine, if_exists="append", index=False, method= DB_CNPJ.psql_insert_copy)
...
if __name__ == '__main__':
    ...
    print(obj.daily_routine(download = True, upload = True, index_base = True))
